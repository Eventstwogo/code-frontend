...
const BookingSuccessContent = () => {
      ...
      try {
        ...
        if (response.status === 200 && response.data) {
          ...
          const bookingDetails: BookingDetails = {
            ...
            slotName: booking.slot || 'Standard Slot',
            startTime: booking.event?.event_time || booking.slot_time?.split(' - ')[0] || '',
            endTime: booking.slot_time?.split(' - ')[1] || '',
  ...
  const handleDownloadTicket = async () => {
    ...
    try {
      ...
      // Use the first seat category label for template selection, or fallback to 'Standard'
      const category = bookingDetails.seatCategories[0]?.label.toLowerCase() || 'standard';
      let templateUrl = `/templates/${category}.html`;
      ...
      // Generate seat categories summary for the template
      const seatCategoriesSummary = bookingDetails.seatCategories
        .map(cat => `${cat.label} (x${cat.num_seats}): $${cat.total_price.toFixed(2)}`)
        .join(', ');
      ...
      ticketHTML = ticketHTML
        ...
        .replace(/\$\{selectedDate\}/g, formatDate(bookingDetails.selectedDate))
        .replace(/\$\{startTime\}/g, bookingDetails.startTime || 'TBD')
        .replace(/\$\{seatCategories\}/g, seatCategoriesSummary || 'General')
  ...
  const generatePDFWithHtml2Canvas = async (ticketHTML: string) => {
    ...
    const fileName = `Events2Go_Ticket_${bookingDetails.seatCategories[0]?.label || 'General'}_${bookingDetails.bookingId}.pdf`;
    pdf.save(fileName);
  };
