...
// type MovieDateSelectorProps = {
//   dates: string[]; // raw dates like "2025-10-07"
//   onDateSelect?: (selectedDate: DateItem, index: number) => void;
// };
...
//   onDateSelect,
// }: MovieDateSelectorProps) {
//   const [formattedDates, setFormattedDates] = useState<DateItem[]>([]);
//   const [selectedIndex, setSelectedIndex] = useState(0);
// console.log(dates)
//   useEffect(() => {
...
//       return {
//         day: dateObj.toLocaleDateString("en-US", { weekday: "short" }), // Tue
//         date: dateObj.toLocaleDateString("en-US", { day: "2-digit" }),  // 07
//         month: dateObj.toLocaleDateString("en-US", { month: "short" }).toUpperCase(), // OCT
//       };
//     });
...
//             key={i}
//             onClick={() => handleDateClick(d, i)}
//             className={`flex flex-col items-center justify-center min-w-[54px] h-[70px] rounded-md transition-all duration-200 flex-shrink-0 border ${
//               selectedIndex === i
...
type MovieDateSelectorProps = {
  dates: string[]; // raw dates like "2025-10-07"
  onDateSelect?: (selectedDate: string, index: number) => void; // Changed to return string
};
...
export default function MovieDateSelector({
  ...
  const handleDateClick = (index: number) => {
    setSelectedIndex(index);
    onDateSelect?.(dates[index], index); // Pass the raw date string
  };
